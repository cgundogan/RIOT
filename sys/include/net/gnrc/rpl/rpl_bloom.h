...
/**
 * @defgroup    net_gnrc_rpl_bloom  RPL-Bloom Extension
 * @ingroup     net_gnrc_rpl
 * @{
 *
 * @file
 * @brief       RPL-Bloom extension data structs
 *
 ...
 */
#define GNRC_RPL_BLOOM_LINKSYM_SIZE               (64)

/**
 * @brief Number of hashes for the Bloom Filter
 */
#define GNRC_RPL_BLOOM_LINKSYM_HASHES_NUMOF       (8)
/** @} */
 ...
 */
/**
 * @brief Parent Announcement RPL DIO control message option
 */
#define GNRC_RPL_OPT_PARENT_ANNOUNCEMENT_LEN  (sizeof(gnrc_rpl_opt_pa_t) - sizeof(gnrc_rpl_opt_t))

/**
 * @brief Neighborhood Announcement RPL DIO control message option
 */
#define GNRC_RPL_OPT_NHOOD_ANNOUNCEMENT_LEN   (sizeof(gnrc_rpl_opt_na_t) - sizeof(gnrc_rpl_opt_t))
/** @} */

/**
 * @brief Most significant bit of the rank
 */
#define GNRC_RPL_BLOOM_MSB(X)      (1U << ((sizeof(X) * 8) - 1))
...
typedef struct __attribute__((packed)) {
    ...
    uint8_t prefix_len;         /**< prefix length of the DODAG-ID */
} gnrc_rpl_opt_pa_t;
...
typedef struct __attribute__((packed)) {
    uint8_t type;                               /**< option type */
    uint8_t length;                             /**< option length without the first two bytes */
    uint8_t bloom[GNRC_RPL_BLOOM_LINKSYM_SIZE/8]; /**< neighborhood bloom filer */
} gnrc_rpl_opt_na_t;

/**
 * @brief RPL-Bloom link symmetry instance extension
 */
typedef struct {
    struct gnrc_rpl_instance *instance;  /**< instance */
    bloom_t nhood_linksym;      /**< neighborhood bloom filter */
    uint8_t bloom_buf[GNRC_RPL_BLOOM_LINKSYM_SIZE/8]; /**< buffer for the bloom filter */
    bool linksym_check_req;     /**< link symmetry check requested */
} gnrc_rpl_bloom_inst_ext_t;

/**
 * @brief RPL-Bloom link symmetry parent extension
 */
typedef struct {
    struct gnrc_rpl_parent *parent;  /**< parent */
    bloom_t nhood_linksym;      /**< neighborhood bloom filter */
    uint8_t bloom_buf[GNRC_RPL_BLOOM_LINKSYM_SIZE/8]; /**< buffer for the bloom filter */
    uint8_t linksym_checks_req; /**< number of link symmetry checks requested */
    timex_t blacklisted_at;     /**< timestamp when the parent was blacklisted */
 ...
 */
void gnrc_rpl_bloom_modify_rank(struct gnrc_rpl_parent *parent);
