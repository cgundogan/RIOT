...
bool ng_rpl_dodag_add(ng_rpl_instance_t *instance, ng_ipv6_addr_t *dodag_id, ng_rpl_dodag_t **dodag)
{
    ...
    *dodag = NULL;
        ...
        if ((ng_rpl_dodags[i].state == 0) && first) {
            *dodag = &ng_rpl_dodags[i];
            ...
            continue;
                ...
                ng_ipv6_addr_equal(&ng_rpl_dodags[i].dodag_id, dodag_id)) {
            DEBUG("DODAG with id: %s does exist\n", ng_ipv6_addr_to_str(addr_str, dodag_id,
                        sizeof(addr_str)));
            *dodag = &ng_rpl_dodags[i];
            return false;
        }
    ...
    if (*dodag != NULL) {
        ...
        ng_rpl_bloom_linksym_neighborhood_init(*dodag);
#endif
        return true;
    }
...
ng_rpl_dodag_t *ng_rpl_dodag_get(ng_rpl_instance_t *instance, ng_ipv6_addr_t *dodag_id)
{
    ...
    LL_FOREACH(instance->dodags, dodag) {
        if (ng_ipv6_addr_equal(&dodag->dodag_id, dodag_id)) {
            return dodag;
        }
    }
    return NULL;
    ...
    *parent = NULL;
    ...
    for (uint8_t i = 0; i < NG_RPL_PARENTS_NUMOF; ++i) {
        ...
        /* return false if parent exists */
                ...
                ng_ipv6_addr_equal(&ng_rpl_parents[i].dodag->dodag_id, &dodag->dodag_id) &&
                ng_ipv6_addr_equal(&ng_rpl_parents[i].addr, addr)) {
            DEBUG("parent with addr: %s does exist\n", ng_ipv6_addr_to_str(addr_str, addr,
                        sizeof(addr_str)));
            *parent = &ng_rpl_parents[i];
            return false;
        }
...
ng_rpl_parent_t *ng_rpl_parent_get(ng_rpl_dodag_t *dodag, ng_ipv6_addr_t *addr)
{
    ...
    LL_FOREACH(dodag->parents, parent) {
        if (ng_ipv6_addr_equal(&parent->addr, addr)) {
            return parent;
        }
    }
    return NULL;
