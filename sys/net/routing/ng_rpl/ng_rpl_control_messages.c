        ...
        ng_ipv6_addr_t *dodag_id)
{
    ...
    hdr = ng_ipv6_hdr_build(pkt, (uint8_t *)src, sizeof(ng_ipv6_addr_t), (uint8_t *)dst,
...
void ng_rpl_send_DIO(ng_rpl_dodag_t *dodag, ng_ipv6_addr_t *destination)
{
    ...
    if (dodag->linksym_check_requested) {
        dodag->linksym_check_requested = false;
        ng_rpl_opt_nhood_ondodag_announcement_t *nha =
            (ng_rpl_opt_nhood_ondodag_announcement_t *) pos;
        nha->type = NG_RPL_OPT_NHOOD_ONDODAG_ANNOUNCEMENT;
        nha->length = NG_RPL_OPT_NHOOD_ONDODAG_ANNOUNCEMENT_LEN;
        memcpy(nha->bloom, dodag->nhood_ondodag->a, NG_RPL_BLOOM_ONDODAG_SIZE/CHAR_BIT);
        pos += sizeof(*nha);
    }
#endif
...
bool _parse_options(ng_rpl_dodag_t *dodag, ng_rpl_opt_t *opt, uint16_t len, ng_ipv6_addr_t *src)
{
    uint16_t l = 0;
...
void ng_rpl_recv_DIO(ng_rpl_dio_t *dio, ng_ipv6_addr_t *src, uint16_t len)
{
    ...
    if (ng_rpl_dodag_add(inst, &dio->dodag_id, &dodag)) {
        ...
        if(!_parse_options(dodag, (ng_rpl_opt_t *)(dio + 1), len, NULL)) {
...
void ng_rpl_recv_DAO(ng_rpl_dao_t *dao, ng_ipv6_addr_t *src, uint16_t len)
{
    ...
    if(!_parse_options(dodag, (ng_rpl_opt_t *) (dao + 1), len, src)) {
        ng_rpl_dodag_remove(dodag);
        return;
    }
