...
static hashfp_t _hashes[GNRC_RPL_BLOOM_LINKSYM_HASHES_NUMOF] = {
...
void gnrc_rpl_bloom_instance_nhood_init(gnrc_rpl_bloom_inst_ext_t *ext)
{
    ...
    bloom_init(&(ext->nhood_linksym), GNRC_RPL_BLOOM_LINKSYM_SIZE, ext->bloom_buf,
               _hashes, GNRC_RPL_BLOOM_LINKSYM_HASHES_NUMOF);
}

void gnrc_rpl_bloom_parent_nhood_init(gnrc_rpl_bloom_parent_ext_t *ext)
{
    ext->linksym_checks_req = 0;
    ext->blacklisted_at = timex_set(0, 0);
    memset(ext->bloom_buf, 0, sizeof(ext->bloom_buf));
...
{
    ipv6_addr_t parent = ext->instance->dodag.dodag_id;
    uint8_t *parent_buf = (uint8_t *) (pa + 1);
    ipv6_addr_init_host(&parent, parent_buf, IPV6_ADDR_BIT_LEN - pa->prefix_len);
    ...
    bloom_add(&(ext->nhood_linksym), (uint8_t *) &(src_suffix.u8[pa->prefix_len / 8]),
...
void gnrc_rpl_bloom_linksym_modify_rank(gnrc_rpl_bloom_parent_ext_t *ext)
{
    ipv6_addr_t *me;
    gnrc_ipv6_netif_find_by_prefix(&me, &(ext->parent->instance->dodag.dodag_id));

    if(!bloom_check(&(ext->nhood_linksym), (uint8_t *)me, sizeof(*me))) {
        ext->parent->rank |= GNRC_RPL_BLOOM_MSB(ext->parent->rank);
    }
    else {
        ext->parent->rank &= ~GNRC_RPL_BLOOM_MSB(ext->parent->rank);
    }
