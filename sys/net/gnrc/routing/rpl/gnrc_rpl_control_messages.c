...
void gnrc_rpl_send_DIO(gnrc_rpl_instance_t *inst, ipv6_addr_t *destination)
{
    ...
    if (inst) {
        ...
        inst->stats.dio_tx_bytes += gnrc_pkt_len(pkt);
    }
#endif
...
gnrc_pktsnip_t *_dis_padding_build(gnrc_pktsnip_t *pkt, int padding)
{
    gnrc_rpl_opt_t *rpl_opt;
    gnrc_pktsnip_t *opt_snip;
    uint8_t *ptr;
    ...
    if ((opt_snip = gnrc_pktbuf_add(pkt, NULL, padding, GNRC_NETTYPE_UNDEF)) == NULL) {
        DEBUG("RPL: BUILD PADDING - no space left in packet buffer\n");
        gnrc_pktbuf_release(pkt);
        return NULL;
    }
    ...
    if (padding > 1) {
        padding -= sizeof(gnrc_rpl_opt_t);
        rpl_opt = opt_snip->data;
        rpl_opt->type = GNRC_RPL_OPT_PADN;
        rpl_opt->length = padding;
        ptr = (ptr + sizeof(gnrc_rpl_opt_t));
    }
                       ...
                       uint8_t req_opts[], uint8_t req_opts_numof)
{
    ...
    if ((pkt = _dis_padding_build(pkt, 4)) == NULL) {
        return;
    }
    ...
    if (inst) {
        ...
        inst->stats.dis_tx_bytes += gnrc_pkt_len(pkt);
    }
#endif
    ...
    if (ipv6_addr_is_multicast(dst)) {
        for (uint8_t i = 0; i < GNRC_RPL_INSTANCES_NUMOF; ++i) {
            ...
            if (gnrc_rpl_instances[i].state != 0) {
                ...
                gnrc_rpl_instances[i].stats.dis_rx_bytes += len;
#endif
...
void gnrc_rpl_send_DAO(gnrc_rpl_instance_t *inst, ipv6_addr_t *destination, uint8_t lifetime)
{
    ...
    if (inst) {
        ...
        inst->stats.dao_tx_bytes += gnrc_pkt_len(pkt);
    }
#endif
...
void gnrc_rpl_send_DAO_ACK(gnrc_rpl_instance_t *inst, ipv6_addr_t *destination, uint8_t seq)
{
    ...
    if (inst) {
        ...
        inst->stats.dao_ack_tx_bytes += gnrc_pkt_len(pkt);
    }
#endif
...
void gnrc_rpl_recv_DAO_ACK(gnrc_rpl_dao_ack_t *dao_ack, kernel_pid_t iface, uint16_t len)
{
    ...
    if ((inst = gnrc_rpl_instance_get(dao_ack->instance_id)) == NULL) {
        ...
        inst->stats.dao_ack_rx_bytes += len;
#endif
